...
addOptional(parser,'cpl',false)   %%% <-- NEW for CPL
...
for t = 1:Tmax_inner
    ...
    if ~cpl                                         %%% <-- NEW for CPL
        ...
        %%% <--row_normalize_ell1 was missing, added it
        Theta = row_normalize_ell1(Lambda);   %%% <-- NEW for CPL
        yt = safe_exp(b*log(Theta'));
    end
    ...
    yt = row_normalize_ell1(yt);  %%% <- this was missing.

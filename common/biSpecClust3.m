...
% Laplacian type spectral clustering.

% KL: could be 1x1 or 1x2: In the latter case KL = [K L] where K is the
% number of row communities and L the number of columns communities. In the
% former case KL = K = L = the number of communtieis of either sides
parser = inputParser;
parser.KeepUnmatched = true;
addOptional(parser,'matched',true) % whether to recover matched communties (K = L) in this case.
...
addOptional(parser,'pert_geom',true) % geometric versus arithmatic scaling for the perturbation
addOptional(parser,'kmeans_rep',100) 
